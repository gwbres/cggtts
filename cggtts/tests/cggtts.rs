...
mod test {
    ...
    fn test_standard_cggtts() {
        ...
        assert_eq!(
            cggtts.rev_date.format("%Y-%m-%d").to_string(),
            String::from("2014-02-20")
        );
        assert_eq!(
            cggtts.rcvr,
            Some(Rcvr {
    ...
    fn parse_standard_data() {
        let test_resources =
            std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR").to_owned() + "/../data/standard");
    ...
    fn parse_advanced_data() {
        let test_resources =
            std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR").to_owned() + "/../data/advanced");
        for entry in std::fs::read_dir(test_resources).unwrap() {
            let entry = entry.unwrap();
            let path = entry.path();
            if !path.is_dir() {
